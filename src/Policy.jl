...
"""
    dispatch_get_order(policy::InventoryOrderingPolicy, state, env, location, lane, product, time)::Int64

`place_orders` (`Simulation.jl`) calls this instead of `get_order` directly:
`policy`'s static type there is the abstract `InventoryOrderingPolicy`
(pulled out of a `Dict{Product, InventoryOrderingPolicy}`), so a plain
`get_order(policy, ...)` call is fully dynamically dispatched. That doesn't
just cost a runtime method lookup - `state`/`env`/`location`/`lane`/`product`
are declared as immutable, non-`isbits` `struct`s (not `mutable struct`s),
so Julia keeps them unboxed/inline in type-stable code and only heap-boxes
them when a value has to flow into a fully dynamic call. Allocation
profiling confirmed this: after switching `Trip.policies` off
`FunctionWrapper` and calling `get_order` directly, `Storage`/`Lane`/`Env`/
`Product` boxing at the call site stayed within noise of the old
`FunctionWrapper` numbers - the `ccall` trampoline wasn't the boxing's
cause, the abstract-typed dynamic call was, and switching *which* dynamic
mechanism triggers it changes nothing.

This hand-writes the union split (an explicit `isa` chain, each followed by
a type assertion) instead of leaving `get_order(policy, ...)` for Julia's
compiler to automatically union-split: automatic splitting only reliably
kicks in for small closed unions (this codebase has 8 concrete
`InventoryOrderingPolicy` subtypes, past where that's dependable). Writing
the branches out by hand isn't relying on the compiler to discover
anything - each branch's type assertion gives the compiler a concretely
resolved call, which compiles as an ordinary static dispatch with no
boxing, regardless of how many total subtypes exist. Any future policy
subtype not listed here still works correctly (falls through to the
dynamic `get_order(policy, ...)` fallback below) - just without this
optimization until it's added to the chain.
"""
function dispatch_get_order(policy::InventoryOrderingPolicy, state::State, env::Env, location::ConcreteNode, lane::Lane, product::Product, time::Int64)::Int64
    if policy isa QuantityOrderingPolicy
        return get_order(policy::QuantityOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa ProductQuantityOrderingPolicy
        return get_order(policy::ProductQuantityOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa OnHandUptoOrderingPolicy
        return get_order(policy::OnHandUptoOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa NetUptoOrderingPolicy
        return get_order(policy::NetUptoOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa NetSSOrderingPolicy
        return get_order(policy::NetSSOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa ForwardCoverageOrderingPolicy
        return get_order(policy::ForwardCoverageOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa BackwardCoverageOrderingPolicy
        return get_order(policy::BackwardCoverageOrderingPolicy, state, env, location, lane, product, time)
    elseif policy isa SingleOrderOrderingPolicy
        return get_order(policy::SingleOrderOrderingPolicy, state, env, location, lane, product, time)
    else
