...
function place_orders(state::State, env::Env, location::Customer, product::Product, time::Int64, orders::Array{OrderLine, 1})
    ...
    if quantity > 0
        ...
        # customer orders are due immediately unless env.customer_backlog opts
        # into letting them queue like every other node's replenishment
        # orders (due_date = typemax(Int64)) instead of being dropped as a
        # lost sale the same period - see Env.customer_backlog's docstring.
